    ...
    //nếu không phải file excel
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        if (ownerType == 1) {
          ...
          let addPersonal = `insert into personal (id, dob, ssn, name, address, phone, gender) \
          ...
          try {
            ...
            if (err.code === "ER_DUP_ENTRY") {
              return res
                .status(400)
                .send(console.log("Duplicate key error: ", err));
            } else {
        ...
        //Thêm vào company
          ...
          let addCompany = `insert into company (id, name, address, phone, taxnum, ownership) 
                values ((SELECT MAX(ID) FROM owner), ?, ?, ?, ?, ?);`;
          let params = [
            data[i]["Tên công ty"],
            data[i]["Địa chỉ"],
        ...
        if (data[i]["Ngày sửa chữa"] !== null) {
          data[i]["Ngày sửa chữa"] = excelDateToJSDate(
            data[i]["Ngày sửa chữa"]
          );
        }

        let addVehicle = `insert into vehicles (licenseId, certId, certDate, regionId, ownerId, brand, model, version, manafractureDate, modified, modifyDate)
              values (?, ?, ?, ?, (SELECT MAX(ID) FROM owner), ?, ?, ?, ?, ?, ?);`;
        let params = [
          data[i]["Biển số"],
          data[i]["Mã đăng ký"],
