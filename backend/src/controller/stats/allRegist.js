...
let allRegist = async (req, res) => {
    ...
    let query = '\
    SELECT r.licenseId, brand, model, version, date, expire, p.name as name, (expire < CURRENT_DATE())\
    as status \
    FROM registry r \
    JOIN vehicles v on r.licenseId = v.licenseId \
    JOIN personal p on v.ownerId = p.id \
    union all \
    SELECT r.licenseId, brand, model, version, date, expire, c.name as name, (expire < CURRENT_DATE()) \
    as status \
    FROM registry r \
    JOIN vehicles v on r.licenseId = v.licenseId \
