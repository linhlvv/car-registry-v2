...
const pageArray = computed(() => {
    const currentPagePosition = Math.floor((props.currentPage - 1)/10)
    const startPoint = currentPagePosition*10 + 1
    let endPoint = currentPagePosition*10 + 10
...
<template>
    <div class="w-full flex items-center justify-center gap-4">
        ...
        <div @click="navigatePage('left')" class="flex items-center cursor-pointer text-[#1d1d1d] border-t-2 border-solid border-transparent hover:border-[#2acc97] duration-150 ease-in-out px-2">
        ...
        <div id="table" class="flex items-center">
            <div v-for="(index) in pageArray" :key="index">
                <div @click="pageClicked(index)" :class="currentPage === index ? 'text-[#2acc97]' : 'hover:border-[#2acc97]'" class="text-[14px] px-4 py-1 cursor-pointer font-semibold border-t-2 border-solid border-transparent duration-150 ease-linear">
                    {{ index }}
                </div>
            </div>
        </div>
        <div @click="navigatePage('right')" class="flex items-center cursor-pointer text-[#1d1d1d] border-t-2 border-solid border-transparent hover:border-[#2acc97] duration-150 ease-in-out px-2">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-7 h-7">
